...
# 量子光学基础：相干态 vs 福克态（Fock State）

## 1. 核心数学定义（算符本征态的差异）

量子光学的基石是产生（$\hat{a}^\dagger$）与湮灭（$\hat{a}$）算符。两种态的本质区别在于它们是**不同算符**的本征态：

> [!note] 福克态（Fock State $|n\rangle$）—— 粒子数算符 $\hat{n}$ 的本征态
> - **定义**：拥有**绝对精确**光子数（$n=0,1,2...$）的量子态。$\hat{n}|n\rangle = n|n\rangle$
> - **与阶梯算符的关系**：它是被阶梯算符**连接**的态。
>     $\hat{a}|n\rangle = \sqrt{n}|n-1\rangle$ （湮灭一个光子，**态发生了根本改变**，变成了 $|n-1\rangle$）
> - **物理意义**：纯粹的“粒子性”体现，无经典波对应。

> [!tip] 相干态（Coherent State $|\alpha\rangle$）—— 湮灭算符 $\hat{a}$ 的本征态
> - **定义**：$\hat{a}|\alpha\rangle = \alpha|\alpha\rangle$ （$\alpha$ 为复数振幅）。
> - **核心特征（鲁棒性）**：对相干态湮灭一个光子后，**量子态保持不变**，依然还是 $|\alpha\rangle$。
> - **物理意义**：量子化了的最完美的**经典电磁波**（如理想单模激光）。

---

## 2. 物理图像：激光为什么是相干态而不是福克态？

这是理解光-电子相互作用的关键。一束宏观飞秒激光的光子态是相干态 $|\alpha\rangle$，而不是具有确定光子数的福克态 $|n\rangle$。

| 物理特性 | 相干态 $|\alpha\rangle$ (The Laser) | 福克态 $|n\rangle$ (Single Photon Source) |
| :--- | :--- | :--- |
| **光子数分布** | **不确定**。服从泊松分布，方差 $\Delta n^2 = \bar{n}$ | **绝对确定**。方差 $\Delta n^2 = 0$ |
| **相位 $\phi$** | **确定的相位**。可以定义波峰波谷。 | **完全不确定**。相位在 $0\sim 2\pi$ 均匀分布。 |
| **电场期望值** | $\langle\alpha|\hat{E}|\alpha\rangle \neq 0$<br>表现出完美的经典正弦振荡。 | $\langle n|\hat{E}|n\rangle = 0$<br>像一团没有确定形状的“量子雾”。 |

> [!abstract] 为什么相干态吸收一个光子后“态不变”？（“大海效应”）
> 相干态是一个无限延展的泊松叠加态。
> **物理直觉**：激光就像一片由水滴组成的海洋（平均水滴数 $\bar{n} = 10^{10}$）。当电子从激光中吸收 1 个光子（湮灭），就像从大海里舀走了一滴水。因为 $\Delta n / \bar{n} \to 0$，宏观电磁场的振幅和相位几乎没有任何可测量的改变。
