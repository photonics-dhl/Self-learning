...
```
电场做功  →  能量守恒  →  麦克斯韦方程  →  E×H 涌现
 J·E       -J·E = ∂u/∂t + ▽·S     代数运算       唯一解
```
...
| 步骤 | 公式 | 含义 |
|------|------|------|
| 做功 | $P = \mathbf{J} \cdot \mathbf{E}$ | 电场给电荷的功率密度 |
| 守恒 | $\mathbf{J} \cdot \mathbf{E} + \frac{\partial u}{\partial t} + \nabla \cdot \mathbf{S} = 0$ | 能量不会凭空消失 |
| 代入 | $\mathbf{E} \cdot \frac{\partial \mathbf{D}}{\partial t} + \mathbf{H} \cdot \frac{\partial \mathbf{B}}{\partial t} = -\nabla \cdot (\mathbf{E} \times \mathbf{H})$ | 麦克斯韦方程相减 |
| **结果** | $\mathbf{S} = \mathbf{E} \times \mathbf{H}$ | 能量通量密度（W/m²） |

## 物理直觉

- $\mathbf{E}$ 方向：电场驱动电荷的方向
- $\mathbf{H}$ 方向：磁场感生旋涡的方向
- $\mathbf{E} \times \mathbf{H}$：能量"被扭曲"的流动方向

> **一句话**：$\mathbf{E} \times \mathbf{H}$ 是能量守恒定律与麦克斯韦方程自洽性要求的唯一解，不是先验假设。
