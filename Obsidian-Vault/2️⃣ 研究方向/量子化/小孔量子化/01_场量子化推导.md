...
在 Lorentz 规范下，矢势推广为四矢量：

$$
A^\mu = (\phi/c,\; \mathbf{A}), \quad \mu = 0, 1, 2, 3
$$

采用度规 $g_{\mu\nu} = \text{diag}(+1, -1, -1, -1)$，即将 Minkowski 度规。

模式展开扩展为：

$$
A^\mu = \int d^3k \sum_{s=0}^{3} N_k \left[a^{(s)}_{k}\,\varepsilon^\mu_{(s)}\,e^{-i(\omega t - \mathbf{k}\cdot\mathbf{r})} + \text{h.c.}\right]
$$

现在有 4 个偏振模式 ($s = 0, 1, 2, 3$)，其中 $s = 0$ 是标量光子（类时偏振），$s = 3$ 是纵向光子。
...
$$
[a^{(\mu)}_{\mathbf{k}}, a^{(\nu)\dagger}_{\mathbf{k}'}] = -g^{\mu\nu}\,\delta^{(3)}(\mathbf{k} - \mathbf{k}')
$$

> [!warning] 负模问题
> 当 $\mu = \nu = 0$ 时，$g^{00} = +1$，所以：
> $$[a^{(0)}, a^{(0)\dagger}] = -\delta^{(3)}(\mathbf{k} - \mathbf{k}')$$
> 这意味着标量光子的态 $a^{(0)\dagger}|0\rangle$ 具有**负模方**（负概率），在物理上不可接受。Gupta-Bleuler 方法通过限制物理态空间来解决这一问题。

### IV.4 Gupta-Bleuler 物理态条件

Gupta-Bleuler 方法不修改对易关系，而是限制 Hilbert 空间。物理态 $|\text{phys}\rangle$ 必须满足：

$$
\partial_\mu \hat{A}^{\mu(+)}|\text{phys}\rangle = 0
$$

其中 $\hat{A}^{\mu(+)}$ 是四矢势的正频部分（只含 $a$ 算符，不含 $a^\dagger$）。

这个条件将标量光子和纵向光子"锁定"在一起——它们在物理态中不能独立出现，只能以特定组合出现，从而消除负模态的影响。

> [!tip] 与 Coulomb 规范的联系
> 在传播波区域，Gupta-Bleuler 条件等价于要求只有横向偏振的光子才能被产生。这是因为 $\partial_\mu A^{\mu(+)} = 0$ 展开后自动给出 $k_\mu \varepsilon^\mu_{(s)} a^{(s)} = 0$，即 Lorentz 条件在算符层面作用于物理态。

### IV.5 倏逝波的特殊性

对于倏逝模式，设 $z$ 方向为衰减方向，波矢分量为：

$$
k^\mu = (\omega/c,\; k_x,\; k_y,\; i\kappa)
$$
...
$$
k_0^2 = \frac{\omega^2}{c^2} = k_\perp^2 - \kappa^2
$$

其中 $k_\perp^2 = k_x^2 + k_y^2$。因此 $\omega/c = \sqrt{k_\perp^2 - \kappa^2}$。

**有效纵向波数**定义为：

$$
k_{\parallel}^{\text{eff}} = \sqrt{k_\perp^2 - \kappa^2} = \frac{\omega}{c} = k_0
$$

> [!note] 关键观察
> 纵向有效波数 $k_{\parallel}^{\text{eff}}$ 恰好等于时间分量 $k_0 = \omega/c$。这不是巧合——它是波矢在复数域中仍满足 $k_\mu k^\mu = 0$（光子质壳条件）的直接结果。

### IV.6 倏逝波的物理态条件

将倏逝波的 $k^\mu$ 代入 Gupta-Bleuler 条件 $k_\mu \varepsilon^\mu_{(s)} a^{(s)} = 0$：

$$
k_0 a^{(0)} - k_1 a^{(1)} - k_2 a^{(2)} - (i\kappa) a^{(3)} = 0
$$

利用 $k_{\parallel}^{\text{eff}} = k_0$ 以及空间分量的横向/纵向分解，上式化简为：
...
> [!tip] 核心结论
> 倏逝波的物理态条件 $\hat{a}^{(0)}|\text{phys}\rangle = \hat{a}^{(3)}|\text{phys}\rangle$ 与传播波的 Gupta-Bleuler 条件**形式完全相同**。这意味着：
>
> 1. Lorentz 规范量子化在倏逝波区域同样有效
> 2. 标量光子与纵向光子的约束关系不变
> 3. 只有"有效横向"自由度是物理的——这与 Coulomb 规范的精神一致
>
> **因此，即使在倏逝波区域，Lorentz 规范的量子化与 Coulomb 规范量子化完全等价。**
...
小孔衍射中，近场区域倏逝波占主导。Section IV 证明了 Coulomb 规范的量子化框架（Sections I–III 建立的）可以安全地推广到倏逝波区域，不需要额外的修正或新的量子化方案。
...
| $\nabla^2\mathbf{A} - \frac{1}{c^2}\frac{\partial^2\mathbf{A}}{\partial t^2} = 0$ | (S1) | 波动方程 |
| $\sum_\lambda \varepsilon_{k\lambda,i}\varepsilon_{k\lambda,j}^* = \delta_{ij} - \frac{k_ik_j}{\|\mathbf{k}\|^2}$ | (S-δ⊥) | 横向 delta 函数 |
| $N_k = \sqrt{\frac{\hbar}{2\varepsilon_0\omega_k(2\pi)^3}}$ | (S-N) | 归一化系数 |
